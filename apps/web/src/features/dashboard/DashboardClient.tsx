...
export function DashboardClient() {
  ...
  const load = () => {
    setError(null);
    void fetch(`${BASE}/dashboard/overview`)
      .then((r) => r.json())
      .then(setOv)
      .catch((e) => setError(String(e)));
    void fetch(`${BASE}/dashboard/gaps`)
      .then((r) => r.json())
      .then(setGaps)
      .catch(() => {});
  };
    ...
    <div className="mx-auto w-full max-w-5xl px-6 py-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <p lang="lo" className="text-muted-foreground text-sm">
            ໂຮງງານ dataset — ສະຖານະ ແລະ ວຽກຄ້າງ
          </p>
        </div>
        <div className="ms-auto flex items-center gap-2">
          ...
          <Link
            href="/studio/ingest"
            className="border-input hover:bg-accent inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm font-medium transition-colors"
          >
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          ...
          sub={`avg ${corpus.avgTokens} tok/chunk`}
          href="/studio/ingest"
        />
        <Stat
          label="Chunks"
          value={String(corpus.chunks)}
          sub={`${corpus.embedded} embedded (${pct(corpus.embedded, corpus.chunks)}%)`}
          href="/studio/review"
        />
        <Stat
          label="Review accepted"
          value={`${pct(corpus.review.accepted, reviewTotal)}%`}
          sub={`${corpus.review.accepted}/${reviewTotal} · ${corpus.review.pending} pending`}
          href="/studio/review"
        />
        <Stat
          label="Verified QA"
          value={`${dataset.qa.verified}/${dataset.qa.total}`}
          sub="export-eligible pairs"
          href="/studio/qa"
        />
      </div>
      <div className="mt-3 grid grid-cols-3 gap-3">
        <Stat
          ...
          value={`${dataset.terms.verified}/${dataset.terms.total}`}
          sub="verified"
          href="/studio/glossary"
        />
        <Stat
          label="Accounts (CoA)"
          value={`${dataset.accounts.verified}/${dataset.accounts.total}`}
          sub="verified"
          href="/studio/knowledge"
        />
        <Stat
          label="Conversations"
          value={String(chat.conversations)}
          sub={`${chat.messages} messages`}
          href="/chat"
        />
      </div>
      ...
      <div className="glass glass-strong mt-4 rounded-2xl p-4">
        ...
        <div className="mt-2 flex h-3 overflow-hidden rounded-full">
          <div
            ...
            style={{ width: `${pct(corpus.review.accepted, reviewTotal)}%` }}
            title={`accepted ${corpus.review.accepted}`}
          />
          <div
            className="bg-amber-400/70"
            style={{ width: `${pct(corpus.review.pending, reviewTotal)}%` }}
            title={`pending ${corpus.review.pending}`}
          />
          <div
            className="bg-destructive/70"
            style={{ width: `${pct(corpus.review.rejected, reviewTotal)}%` }}
            title={`rejected ${corpus.review.rejected}`}
          />
        </div>
        ...
        <div className="glass glass-strong rounded-2xl p-4">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            {gaps && gaps.abstained.length > 0 && (
              <Badge variant="secondary">{gaps.abstained.length}</Badge>
            )}
          </div>
          {!gaps || gaps.abstained.length === 0 ? (
            <p lang="lo" className="text-muted-foreground mt-3 text-sm">
              ບໍ່ມີ — AI ຕອບໄດ້ທຸກຄຳຖາມທີ່ຜ່ານມາ
            </p>
          ) : (
            <div className="mt-2 space-y-2">
              {gaps.abstained.map((g, i) => (
                <div key={i} className="border-border rounded-lg border px-3 py-2">
                  ...
                  <p lang="lo" className="text-muted-foreground mt-0.5 line-clamp-1 text-xs">
                    {g.answer}
                  </p>
                  <Link
                    href="/studio/teach"
          ...
          {gaps && gaps.reported.length > 0 && (
            <div className="border-border mt-3 border-t pt-2">
              <p className="text-muted-foreground text-xs font-medium">
                reported chunks ({gaps.reported.length})
              </p>
              {gaps.reported.map((r) => (
              ...
              {ov.activity.map((a, i) => (
                <div key={i} className="flex items-baseline gap-2 text-xs">
                  ...
                  <Badge
                    ...
                    className={cn(
                      "shrink-0",
                      a.eventType.includes("deleted") && "text-destructive",
                    )}
                  >
                    {a.eventType}
                  </Badge>
